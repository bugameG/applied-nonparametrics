---
title: "Non-parametric Methods - Problems"
author: "Gift Pamba"
date: "May-June 2026"
output:
  html_document:
    toc: true
    toc_float: true
    theme: darkly
  pdf_document:
    toc: true
---

```{r setup, include=FALSE}
library(tidyverse)
library(DescTools)
library(kableExtra)
library(broom)

```

# 1. Comparing two related samples

## ONE

A teacher wished to determine if providing a bilingual dictionary to students with limited English proficiency improves math test scores. A small class of students **(n = 10)** was selected. Students were given two math tests. Each test covered the same type of math content; however, students were provided a bilingual dictionary on the second test. The data in `Table 3.10` represent the students’ performance on each math test.

| student | Math test without a billingual dictionary | Math test with a billingual dictionary |
|-------------------|---------------------------|--------------------------|
| 1 | 30 | 39 |
| 2 | 56 | 46 |
| 3 | 48 | 37 |
| 4 | 47 | 44 |
| 5 | 43 | 32 |
| 6 | 45 | 39 |
| 7 | 36 | 41 |
| 8 | 44 | 40 |
| 9 | 44 | 38 |
| 10 | 40 | 46 |

: Table 3.10

```{r, echo=FALSE, message=FALSE, warning=FALSE}
tab1 <- data.frame(
  "student"=seq(1,10,1),
  "before"=c(30,56,48,47,43,45,36,44,44,40),
  "after"=c(39,46,37,44,32,39,41,40,38,46)
)
```

Use a **one-tailed Wilcoxon signed rank** test and a **one-tailed sign test** to determine which testing condition resulted in higher scores. Use $\alpha = 0.05$. Report your findings

$${H}_{0}:\text{The billingual dictionary has no effect on math test scores}\\ \\ {H}_{1}:\text{Math test scores are higher without the billingual dictionary}$$



```{r, echo=FALSE, message=FALSE, warning=FALSE}
# One-tailed Wilcoxon signed-rank test
dictionary <- wilcox.test(x = tab1$before, y = tab1$after,
                          alternative = "greater",
                          paired = TRUE)

# One-tailed Sign Test
dictionary2 <- SignTest(x = tab1$before, y = tab1$after,
                        alternative = "greater")

cat(
  paste0("One-tailed Wilcoxon signed-rank test","\n",
        "---------------------------------------","\n",
        "p-value: ", round(dictionary$p.value,4), "\n",
        "Decision: ", ifelse(dictionary$p.value < 0.05,
                             yes = "Reject the null hypothesis",
                             no = "Fail to reject the null hypothesis"), "\n","\n",
        "One-tailed Sign Test","\n",
        "--------------------------","\n",
        "p-value: ", round(dictionary2$p.value,4), "\n",
        "Decision: ", ifelse(dictionary2$p.value < 0.05,
                             yes = "Reject the null hypothesis",
                             no = "Fail to reject the null hypothesis")
        )
)
```


> A **one-tailed Wilcoxon signed-rank test** and a **one-tailed sign test** were conducted to determine whether students performed better on the math test without a bilingual dictionary, with $\alpha = 0.05$ and $n = 10$. Median scores were $44.0 \text{ (without dictionary) } \text{ and } 39.5 \text{ (with dictionary) }$. For the Wilcoxon signed-rank test, the result was **non-significant** $(V = `r dictionary$statistic`, p = `r round(dictionary$p.value, 4)`)$. 
The sign test yielded **7 positive differences** and **3 negative differences**, also **non-significant** $(p = `r round(dictionary2$p.value, 4)`)$. Both tests fail to reject ${H}_{0}$ — there is insufficient evidence to conclude that students scored higher without the bilingual dictionary.


## TWO

A research study was done to investigate the influence of being alone at night on the human male heart rate.Two mean were sent into a wooded area, one at a time, at night, for 20 minutes. They had a heart monitor to record their pulse rate. The second night, the same men were sent into a similar wooded area accompanied by a companion. Thei pulse rate was recorded again. The researcher wanted to see if having a companion would change their pulse rate. The median pulse rates are recorded in `Table 3.11`. Use a **two-tailed Wilcoxon signed-rank test** and a **two-tailed signed test** to determine which condition produced a higher pulse rate. Use $\alpha = 0.05$. Report your findings

| participant | median rate alone | median rate with companion |
|-------------|-------------------|----------------------------|
| A           | 88                | 72                         |
| B           | 77                | 74                         |
| C           | 91                | 80                         |
| D           | 70                | 77                         |
| E           | 80                | 71                         |
| F           | 85                | 83                         |
| G           | 90                | 80                         |
| H           | 82                | 91                         |
| I           | 93                | 86                         |
| J           | 75                | 69                         |

: Table 3.11

$${H}_{0}:\text{There is no diference in pulse rate between being alone and having a companion}\\ \\ {H}_{1}:\text{There is a difference in pulse rate between the two conditions}$$


```{r, echo=FALSE, message=FALSE, warning=FALSE}
pulse <- data.frame(
  "participant" = LETTERS[1:10],
  "without" = c(88,77,91,70,80,85,90,82,93,75),
  "with" = c(72,74,80,77,71,83,80,91,86,69))

# Two-tailed Wilcoxon signed-rank test
hr <- wilcox.test(x = pulse$without, y = pulse$with,
                  alternative = "two.sided", 
                  paired = TRUE)

# Two-tailed Sign test
hr2 <- SignTest(x = pulse$without, y = pulse$with,
                  alternative = "two.sided")

cat(
  paste0("Two-tailed Wilcoxon signed-rank test","\n",
        "---------------------------------------","\n",
        "p-value: ", round(hr$p.value,4), "\n",
        "Decision: ", ifelse(hr$p.value < 0.05,
                             yes = "Reject the null hypothesis",
                             no = "Fail to reject the null hypothesis"), "\n","\n",
        "Two-tailed Sign Test","\n",
        "--------------------------","\n",
        "p-value: ", round(hr2$p.value,4), "\n",
        "Decision: ", ifelse(hr2$p.value < 0.05,
                             yes = "Reject the null hypothesis",
                             no = "Fail to reject the null hypothesis")
        )
)
```
> **Two-tailed Wilcoxon signed-rank** and **sign tests** were conducted to assess whether having a companion affected heart rate among $n = 10$ male participants, with $\alpha = 0.05$. Median pulse rates were $83.5 \text{ bpm  (alone) } and 78.5 \text{bpm  (with companion) }$. The **Wilcoxon signed-rank test** was **non-significant** $(V = `r hr$statistic `, p = `r round(hr$p.value,4)`)$.
The **sign test** recorded **8 positive** and **2 negative** differences, also **non-significant** $(p = `r round(hr2$p.value,4)`)$. Both tests fail to reject ${H}_{0}$ — the data provides insufficient evidence that suggests a difference in pulse rate between the two conditions.


## THREE

A researcher conducts a pilot study to compare two treatments to help obese female teenagers lose weight. She tests each individual in two different treatment conditions. The data in `Table 3.12` provides the number of pounds that each participant lost.

|                 |                 |                 |
|-----------------|-----------------|-----------------|
| **participant** | **treatment 1** | **treatment 2** |
| 1               | 10              | 18              |
| 2               | 20              | 12              |
| 3               | 15              | 16              |
| 4               | 9               | 7               |
| 5               | 18              | 21              |
| 6               | 11              | 17              |
| 7               | 6               | 13              |
| 8               | 12              | 14              |

: Table 3.12

Use a **two-tailed Wilcoxon signed-rank test** and a **two-tailed sign test** to determine which treatment resulted in greater weight loss. Use $\alpha = 0.05$. Report your findings


$${H}_{0}:\text{There is no difference in weight loss between treatment 1 and 2}\\ \\ {H}_{1}:\text{There is a difference in weight loss between the two treatments}$$

```{r, echo = FALSE, message = FALSE, warning = FALSE}
wl <- data.frame(
  "participant" = 1:8,
  "t1" = c(10,20,15,9,18,11,6,12),
  "t2" = c(18,12,16,7,21,17,13,14))

# Two-tailed Wilcoxon signed-rank test
wl_wsr <- wilcox.test(x = wl$t1, y = wl$t2,
                      alternative = "two.sided", paired = TRUE)
# Two-tailed sign test
wl_sign <- SignTest(x = wl$t1, y = wl$t2,
                    alternative = "two.sided")

cat(
  paste0("Two-tailed Wilcoxon signed-rank test","\n",
        "---------------------------------------","\n",
        "p-value: ", round(wl_wsr$p.value,4), "\n",
        "Decision: ", ifelse(wl_wsr$p.value < 0.05,
                             yes = "Reject the null hypothesis",
                             no = "Fail to reject the null hypothesis"), "\n","\n",
        "Two-tailed Sign Test","\n",
        "--------------------------","\n",
        "p-value: ", round(wl_sign$p.value,4), "\n",
        "Decision: ", ifelse(wl_sign$p.value < 0.05,
                             yes = "Reject the null hypothesis",
                             no = "Fail to reject the null hypothesis")
        )
)
```

> **Two-tailed Wilcoxon signed-rank** and **sign tests** were applied to compare weight loss outcomes between two treatments among $n = 8$ female participants, with $\alpha = 0.05$. Median weight loss was $11.5 \text{ lbs under treatment 1 and }15.0 \text{ lbs under treatment 2 }$. The **Wilcoxon signed-rank test** was **non-significant** $(V = `r wl_wsr$statistic`, p = `r round(wl_wsr$p.value,4)`)$. The sign test yielded only **2 positive** differences and **6 negative** differences, also **non-significant** $(p = `r round(wl_sign$p.value,4)`)$. Both tests fail to reject ${H}_{0}$- there is no statistically significant difference in weight loss between the two treatments, although the descriptive pattern leans in favour of treatment 2.


## FOUR

Twenty participants in an exercise program were measured on the number of sit-ups they could do before physical exercise (first count) and the number they could do after they had done at least 45 minutes of other physical exercise.(second count) `Table 3.13` shows the results for 20 participants obtained during two separate physical exercise sessions. Determine the **Effect Size** for a calculated z-score

```{r, echo = FALSE, message = FALSE, warning = FALSE}
ex <- data.frame(
"participant" = seq(1,20,1),
"first" = c(18,19,20,29,15,22,21,30,22,11,20,21,21,20,18,27,24,13,10,10),
"second" = c(28,18,28,20,30,25,28,18,27,30,24,27,10,40,20,14,29,30,24,36)
)

# Sample size
n_ex <- nrow(ex)

# Wilcoxon signed-rank hypothesis test for paired samples
ex_wsr <- wilcox.test(x = ex$first, y = ex$second,
                      alternative = "two.sided",
                      paired = TRUE)

# Test statistic
ex_wsr_t <- ex_wsr$statistic

# Normal approximation
## E[T]
ex_wsr_tmean <- ((n_ex)*(n_ex+1))/4

## sd[T]
ex_wsr_tsd <- sqrt((n_ex*(n_ex+1)*(2*n_ex+1))/24)

## z-variate
z_ex <- ((ex_wsr_t-ex_wsr_tmean)/ex_wsr_tsd)

## Effect size
es_ex <- abs(z_ex)/sqrt(n_ex)

cat(paste0("Effect size: ", round(es_ex, 4)))
```
> This constitutes a **medium-to-large effect**, indicating that the exercise session had a practically meaningful impact on sit-up performance. 

## FIVE

A school is trying to get more students to participate in activities that will make learning more desirable. `Table 3.14` shows the number of activities that each of the 10 students in one class participated in last year before a new activity program was implemented and this year after it was implemented. Construct a 95% confidence interval based on the **Wilcoxon signed-rank test** to determine whether the new activity program had a significant positive effect on the student participation

$${H}_{0}:\text{The new activity program has no effect on student participation} \\ \\ {H}_{1}:\text{The new activity program has a significant positive effect on student participation}$$

```{r, echo=FALSE, message=FALSE, warning=FALSE}
activity <- data.frame(
  "participant" = seq(1,10,1),
  "lastyr" = c(18,22,10,25,16,14,21,13,28,12),
  "thisyr" = c(20,28,18,23,20,21,17,18,22,21)
)

# Wilcoxon signed-rank hypothesis test
act_wsr <- wilcox.test(x = activity$lastyr, y = activity$thisyr,
                       alternative = "two.sided", paired = TRUE,
                       conf.int = TRUE)

cat(
  paste0("[",round(act_wsr$conf.int[1],4),",",round(act_wsr$conf.int[2],4),"]")
)
```

> Since the above interval contains zero, we fail to reject ${H}_{0}$— there is insufficient evidence at $\alpha = 0.05$ to conclude that the new activity program had a significant positive effect on student participation.

## SIX

*(ANSM 4.3)*

Samples of cream from each of ten dairies (A to J) are each divided into two portions. One portion from each is sent to laboratory I, the other to Laboratory II, for bacterium counts. The counts (thousands bacteria ${ml}^{-1}$) are:

| Dairy | Lab I | Lab II |
|-------|-------|--------|
| A     | 11.7  | 10.9   |
| B     | 12.1  | 11.9   |
| C     | 13.3  | 13.4   |
| D     | 15.1  | 15.4   |
| E     | 15.9  | 14.8   |
| F     | 15.3  | 14.8   |
| G     | 11.9  | 12.3   |
| H     | 16.2  | 15.0   |
| I     | 15.1  | 14.2   |
| J     | 13.6  | 13.1   |

```{r, echo = FALSE, message = FALSE, warning = FALSE}
bacteria <- data.frame(
  "dairy" = LETTERS[1:10],
  "l1" = c(11.7,12.1,13.3,15.1,15.9,15.3,11.9,16.2,15.1,13.6),
  "l2" = c(10.9,11.9,13.4,15.4,14.8,14.8,12.3,15,14.2,13.1)
)
```

Use the **Wilcoxon signed-rank test** to assess the evidence for any consistent difference between laboratories for subsamples from the same dairy. Obtain also 95 and 99 percent confidence intervals for the mean and compare these with the intervals using the optimal method when normality is assumed

$${H}_{0}:\text{There is no consisitent difference in bacteria counts between laboratory I and laboratory II}\\ \\ {H}_{1}:\text{There is a consistent difference in bacteria counts between the two laboratories}$$

```{r, echo = FALSE, message = FALSE, warning = FALSE}
# Two-tailed Wilcoxon signed rank test
bacteria_wsr1 <- wilcox.test(x = bacteria$l1, y = bacteria$l2,
                          paired = TRUE, exact = FALSE, conf.int = TRUE,
                          alternative = "two.sided", conf.level = c(0.95))
# 95% confidence interval
lower_ci95 <- bacteria_wsr1$conf.int[1] |> round(4)
upper_ci95 <- bacteria_wsr1$conf.int[2] |> round(4)


# Two-tailed Wilcoxon signed rank test-[99% confidence interval]
bacteria_wsr2 <- wilcox.test(x = bacteria$l1, y = bacteria$l2,
                          paired = TRUE, exact = FALSE, conf.int = TRUE,
                          alternative = "two.sided", conf.level = c(0.99))

# 99% confidence interval
lower_ci99 <- bacteria_wsr2$conf.int[1] |> round(2)
upper_ci99 <- bacteria_wsr2$conf.int[2] |> round(2)

cat(
  paste0("Two-tailed Wilcoxon signed-rank test","\n",
        "------------------------------------------------","\n",
        "p-value: ", round(bacteria_wsr1$p.value,4), "\n",
        "Decision: ", ifelse(bacteria_wsr1$p.value < 0.05,
                             yes = "Reject the null hypothesis",
                             no = "Fail to reject the null hypothesis"))
)

```
> A **two-tailed Wilcoxon signed-rank** test was conducted to assess whether a consistent difference existed between bacteria counts $(\text{thousands }{ml}^{-1})$ recorded by two laboratories across $n = 10$ dairy subsamples, with $\alpha = 0.05$
. Median counts were $14.35 \text{ (Lab I) and }13.80\text{ (Lab II) }$. The test was marginally **non-significant** $(V = `r bacteria_wsr1$statistic`, p = `r round(bacteria_wsr1$p.value,4)`)$. We fail to reject ${H}_{0}$- the evidence for a consistent laboratory difference falls just short of the $5\%$ threshold. The **Hodges-Lehmann** estimate of the location shift was $0.450\text{ thousand bacteria } {ml}^{−1}$

-------------------------------

> The nonparametric 95% confidence interval for the difference was $[`r lower_ci95`,`r upper_ci95`]$ and the 99% confidence interval was $[`r lower_ci99`,`r upper_ci99`]$, both straddling zero and consistent with the test decision



## SEVEN

*(ANSM 4.6)*

For each of nine matched pairs of students, one student is allocated to a series of lectures and the other to appropriate computer assisted learning (CAL) material. At the end of the course, the students are given the same examination paper. The marks achieved (out of 100) are:

| pair | CAL | lectures |
|------|-----|----------|
| 1    | 50  | 25       |
| 2    | 56  | 58       |
| 3    | 51  | 65       |
| 4    | 46  | 38       |
| 5    | 88  | 91       |
| 6    | 79  | 32       |
| 7    | 81  | 31       |
| 8    | 95  | 13       |
| 9    | 73  | 49       |

```{r, echo = FALSE, message = FALSE, warning = FALSE}
lvc <- data.frame(
  "pair" = 1:9,
  "CAL" = c(50,56,51,46,88,79,81,95,73),
  "lectures" = c(25,58,65,38,91,32,31,13,49)
)
```

$${H}_{0}:\text{There is no difference in examination performance between CAL and lecture-based learning}\\ \\ {H}_{1}:\text{There is a difference in examination performance between the two methods}$$

Analyze these results by what you consider the most appropriate parametric or nonparametric methods to determine whether or not they provide acceptable evidence that CAL materials lead to better examinations results

```{r, echo = FALSE, message = FALSE, warning = FALSE}
lvc_wsr <- wilcox.test(x = lvc$CAL,
                       y = lvc$lectures, paired = TRUE, alternative = "two.sided")

cat(
  paste0("Two-tailed Wilcoxon signed-rank test","\n",
        "------------------------------------------------","\n",
        "p-value: ", round(lvc_wsr$p.value,4), "\n",
        "Decision: ", ifelse(lvc_wsr$p.value < 0.05,
                             yes = "Reject the null hypothesis",
                             no = "Fail to reject the null hypothesis"))
)

```
> A **two-tailed Wilcoxon signed-rank** test was used to compare examination marks (out of 100) between students assigned to computer-assisted learning (CAL) and those assigned to lectures, across $n = 9$ matched pairs, with $\alpha = 0.05$
. Median marks were $73\text{ (CAL) and }38\text{ (lectures) }$. Of the $9$ pairs, $6$ showed higher marks under CAL and $3$ under lectures. Despite this descriptive advantage, the test was **non-significant** $(V = `r lvc_wsr$statistic`, p = `r round(lvc_wsr$p.value,4)`)$. We fail to reject ${H}_{0}$— there is insufficient evidence at the $5\%$ level to conclude that CAL and lecture-based learning produce different examination outcomes.


------------------------------------------------------------------------

# 2. Comparing two unrelated samples

## ONE

The data in `Table 4.8` were obtained from a reading-level test for 1st grade children. Compare the performance gains of the two different methods for teaching reading.

| Method     | Gain score | Method      | Gain score |
|------------|------------|-------------|------------|
| One on one | 16         | Small group | 11         |
| One on one | 13         | Small group | 2          |
| One on one | 16         | Small group | 10         |
| One on one | 16         | Small group | 4          |
| One on one | 13         | Small group | 9          |
| One on one | 9          | Small group | 8          |
| One on one | 12         | Small group | 5          |
| One on one | 12         | Small group | 6          |
| One on one | 20         | Small group | 4          |
| One on one | 17         | Small group | 16         |

: Table 4.8

```{r, echo = FALSE, message = FALSE, warning = FALSE}
firstgrade <- data.frame(
  "method" = c(rep("ono", 10), rep("sg", 10)),
  "score" = c(16,13,16,16,13,9,12,12,20,17,
              11,2,10,4,9,8,5,6,4,16)
)
```

Use **two-tailed Mann-Whitney U** and **Kolmogorov-Smirnov two-sample** tests to determine which method was better for teaching reading. Set $\alpha = 0.05$. Report your findings

### a

```{r, echo = FALSE, message = FALSE, warning = FALSE}
fg_mw <- wilcox.test(x = firstgrade$score[firstgrade$method=="ono"],
                     y = firstgrade$score[firstgrade$method=="sg"],
                     alternative = "two.sided", paired = FALSE)
```


$${H}_{0}:\text{}\\ \\ {H}_{1}:\text{}$$

### b

```{r, echo = FALSE, message = FALSE, warning = FALSE}
fg_ks <- ks.test(x = firstgrade$score[firstgrade$method=="ono"],
                 y = firstgrade$score[firstgrade$method=="sg"],
                     alternative = "two.sided")
```

## TWO

A research study was conducted to see if an active involvement in a hobby has a positive effect on the health of a person who retires after age 65. The data in `Table 4.9` describes the health (number of doctor visits in 1 year) for participants who are involved in a hobby almost daily and those who are not.

| No hobby group | Hobby group |
|----------------|-------------|
| 12             | 9           |
| 15             | 5           |
| 8              | 10          |
| 11             | 3           |
| 9              | 4           |
| 17             | 2           |

: Table 4.9

```{r, echo = FALSE, message = FALSE, warning = FALSE}
hobby <- data.frame(
  "group" = c(rep("nohobby",6), rep("hobby",6)),
  "visits" = c(12,15,8,11,9,17,
               9,5,10,3,4,2)
)
```

Use **one-tailed Mann-Whitney U** and **Kolmogorov-Smirnov two-sample** tests to determine whether the hobby tends to reduce the need for doctor visits. Set $\alpha = 0.05$. Report your findings

### a

```{r, echo = FALSE, message = FALSE, warning = FALSE}
hobby_mw <- wilcox.test(x = hobby$visits[hobby$group=="nohobby"],
                        y = hobby$visits[hobby$group=="hobby"],
                        alternative = "greater", paired = FALSE)
```

### b

```{r, echo = FALSE, message = FALSE, warning = FALSE}
hobby_ks <- ks.test(x = hobby$visits[hobby$group=="nohobby"],
                    y = hobby$visits[hobby$group=="hobby"],
                    alternative = "two.sided")
```

## THREE

| Method 1 | Method 2 |
|----------|----------|
| 53       | 91       |
| 41       | 18       |
| 17       | 14       |
| 45       | 21       |
| 44       | 23       |
| 12       | 99       |
| 49       | 16       |
| 50       | 10       |

: Table 4.10

`Table 4.10` shows assessment scores of two different classes who are being taught computer skills using two different methods

```{r, echo = FALSE, message = FALSE, warning = FALSE}
compskills <- data.frame(
  "method" = c(rep("method1", 8), rep("method2", 8)),
  "score" = c(53,41,17,45,44,12,49,50,
               91,18,14,21,23,99,16,10)
  )
```

Use **two-tailed Mann-Whitney U** and **Kolmogorov-Smirnov two-sample** tests to determine which method was better for teaching compute skills. Set $\alpha = 0.05$. Report your findings

### a

```{r, echo = FALSE, message = FALSE, warning = FALSE}
comp_mw <- wilcox.test(x = compskills$score[compskills$method == "method1"],
                       y = compskills$score[compskills$method == "method2"],
                       alternative = "two.sided", paired = FALSE)
```

### b

```{r, echo = FALSE, message = FALSE, warning = FALSE}
comp_ks <- ks.test(x = compskills$score[compskills$method == "method1"],
                   y = compskills$score[compskills$method == "method2"],
                   alternative = "two.sided")
```

## FOUR

Two methods of teaching were compared. Method 1 uses the computer to interact with the student and diagnose and remediate the student based on misconceptions. Method 2 was taught using workbooks in classroom groups. `Table 4.11` shows the data obtained on an assessment after 6 weeks of instruction. Calculate the ES using the z-score from the analysis.

|          |          |
|----------|----------|
| Method 1 | Method 2 |
| 27       | 9        |
| 38       | 42       |
| 15       | 21       |
| 85       | 83       |
| 36       | 110      |
| 95       | 19       |
| 93       | 29       |
| 57       | 40       |
| 63       | 30       |
| 81       | 23       |
| 65       | 18       |
| 77       | 32       |
| 59       | 101      |
| 89       | 7        |
| 41       | 50       |
| 26       | 37       |
| 102      | 22       |
| 55       | 71       |
| 46       | 16       |
| 82       | 45       |
| 24       | 35       |
| 87       | 28       |
| 66       | 91       |
| 12       | 86       |
| 90       | 20       |

: Table 4.11

```{r, echo = FALSE, message = FALSE, warning = FALSE}
comp2 <- data.frame(
  "method" = c(rep("method1",25), rep("method2", 25)),
"score" = c(27,38,15,85,36,95,93,57,63,81,65,77,59,89,41,26,102,55,46,82,24,87,66,12,90,
9,42,21,83,110,19,29,40,30,23,18,32,101,7,50,37,22,71,16,45,35,28,91,86,20)
)

# Sample size
n_comp2 <- 25

# Wilcoxon rank sum hypothesis test for paired samples
comp2_wsr <- wilcox.test(x = comp2$score[comp2$method == "method1"], 
                         y = comp2$score[comp2$method == "method2"],
                      alternative = "two.sided", paired = FALSE)

# Test statistic
comp2_wsr_t <- comp2_wsr$statistic

# Normal approximation
## E[T]
comp2_wsr_tmean <- ((n_comp2)*(n_comp2+1))/4

## sd[T]
comp2_wsr_tsd <- sqrt((n_comp2*(n_comp2+1)*(2*n_comp2+1))/24)

## z-variate
z_comp2 <- ((comp2_wsr_t-comp2_wsr_tmean)/comp2_wsr_tsd)

## Effect size
es_comp2 <- abs(z_comp2)/sqrt(n_comp2)

cat(paste0("Effect size: ", round(es_comp2, 4)))
```

## FIVE

Two methods were used to provide instruction in Science for 7th Grade. Method 1 included a laboratory each week and method 2 has only classroom work with lecture and worksheets. `Table 4.12` shows end-of-term test performance for the two methods. Construct a 95% median confidence interval based on the difference between two independent samples to compare the two methods.

| Method 1 | Method 2 |
|----------|----------|
| 15       | 8        |
| 23       | 15       |
| 9        | 10       |
| 12       | 13       |
| 18       | 17       |
| 22       | 5        |
| 17       | 18       |
| 20       | 7        |

: Table 4.12

```{r, echo = FALSE, message = FALSE, warning = FALSE}
lab <- data.frame(
  "method" = c(rep("1", 8), rep("2", 8)),
  "score" = c(15,23,9,12,18,22,17,20,
              8,15,10,13,17,5,18,7)
)
```

## SIX

*(ANSM 5.5)*

An alloy is composed of zinc, copper and tin. It may be made at one of two temperatures H (higher) or L (lower). We wish to know if one temperature produces a harder alloy. A sample is taken from each of 9 batches at L and 7 at H. To arrange them in ascending order of hardness, all specimens are scraped against one another to see which makes a deeper scratch (a deeper scratch indicates a softer specimen). On this basis the specimens are ranked 1 (softest) to 16 (hardest) with the results given below. Should we reject the hypothesis that hardness is unaffected by temperature?

| rank | temperature |
|------|-------------|
| 1    | H           |
| 2    | L           |
| 3    | H           |
| 4    | H           |
| 5    | H           |
| 6    | L           |
| 7    | H           |
| 8    | L           |
| 9    | L           |
| 10   | H           |
| 11   | H           |
| 12   | L           |
| 13   | L           |
| 14   | L           |
| 15   | L           |
| 16   | L           |

```{r, echo = FALSE, message = FALSE, warning = FALSE}
metal_temp <- data.frame(
  "temp" = c("H","L","H","H","H","L","H","L","L","H","H","L","L","L","L","L"),
  "rank" = 1:16
)

metal_temp_wrs <- wilcox.test(x = metal_temp$rank[metal_temp$temp == "H"],
                              y = metal_temp$rank[metal_temp$temp == "L"],
                              alternative = "two.sided", exact = TRUE,
                              paired = FALSE)
```

## SEVEN

*(ANSM 5.9)*

Hotpot stoves use a standard oven insulation. To test its effectiveness they take random samples from the production line and heat the ovens selected to $400^{0}C$, noting the time taken to cool to $350^{0}C$, after switching off. For a sample of 8 ovens the times in minutes are: $$15.7,\ 14.8,\ 14.2,\ 16.1,\ 15.3,\ 13.9,\ 17.2,\ 14.9$$ They decide to explore a cheaper insulation, and using this on a sample of 9 the times taken for the same temperature drop are: $$13.7,\ 14.1,\ 14.7,\ 15.4,\ 15.6,\ 14.4,\ 12.9,\ 15.1,\ 14.0$$ Are the firm justified in asserting there is no real evidence of a different rate of heat loss? Obtain a 95 per cent confidence limit for the difference in median heat loss with and without a normality assumption. Comment critically on any differences between your conclusions

```{r, echo = FALSE, message = FALSE, warning = FALSE}
hotpot <- data.frame(
  "insulation" = c(rep("1", 8), rep("2", 9)),
  "minutes" = c(15.7,14.8,14.2,16.1,15.3,13.9,17.2,14.9,
                13.7,14.1,14.7,15.4,15.6,14.4,12.9,15.1,14)
)

hotpot_wrs <- wilcox.test(x = hotpot$minutes[hotpot$insulation == "1"],
                          y = hotpot$minutes[hotpot$insulation == "2"],
                          paired = FALSE, alternative = "two.sided",
                          exact = FALSE, conf.int = TRUE)
```

## EIGHT

*(ANSM 5.14)*

A psychologist notes total time (in seconds) needed to perform a series of simple manual tasks for each of eight children with learning difficulties and seven children without learning difficulties. The times are:

| without difficulties | with difficulties |
|----------------------|-------------------|
| 204                  | 243               |
| 218                  | 228               |
| 197                  | 261               |
| 183                  | 202               |
| 227                  | 343               |
| 233                  | 242               |
| 191                  | 220               |
|                      | 239               |

Use a Smirnov test to find whether the psychologist is justified in asserting these samples are likely to be from different populations.

```{r, echo = FALSE, message = FALSE, warning = FALSE}
diff <- data.frame(
  "grp" = c(rep("without", 7), rep("with", 8)),
  "time" = c(204,218,197,183,227,233,191,
             243,228,261,202,343,242,220,239)
)

diff_ks <- ks.test(
  x = diff$time[diff$grp == "with"],
  y = diff$time[diff$grp == "without"], 
  alternative = "two.sided", exact = TRUE)
```

## NINE

*(ANSM 5.18)*

Carter and Hubert (1985) give data for percentage variation in blood sugar over 1-hour periods for rabbits given two different dose levels of a drug. Is there evidence of a response difference between levels?

| dose 1 | dose 2 |
|--------|--------|
| 0.21   | 1.59   |
| -16.20 | 2.66   |
| -10.10 | -6.27  |
| -8.67  | -2.32  |
| -11.13 | -10.87 |
| 1.96   | 7.23   |
| -10.19 | -3.76  |
| -15.87 | 3.02   |
| -12.81 | 15.01  |

```{r, echo = FALSE, message = FALSE, warning = FALSE}
dose <- data.frame(
  "dose" = c(rep("I", 9), rep("II", 9)),
  "pct_var"= c(0.21,-16.20,-10.10,-8.67,-11.13,1.96,-10.19,-15.87,-12.81,
               1.59,2.66,-6.27,-2.32,-10.87,7.23,-3.76,3.02,15.01)
)
```

## TEN

*(ANSM 5.22)*

The following data are DMF scores for `34` male and `54` female first-year dental students. The DMF score is the total of the numbers of decayed + missing + filled teeth.

| gender | DMF |
|--------|-----|
| male   | 8   |
| male   | 6   |
| male   | 4   |
| male   | 2   |
| male   | 10  |
| male   | 5   |
| male   | 6   |
| male   | 6   |
| male   | 19  |
| male   | 4   |
| male   | 10  |
| male   | 4   |
| male   | 10  |
| male   | 12  |
| male   | 7   |
| male   | 2   |
| male   | 5   |
| male   | 1   |
| male   | 8   |
| male   | 2   |
| male   | 0   |
| male   | 7   |
| male   | 6   |
| male   | 4   |
| male   | 4   |
| male   | 11  |
| male   | 2   |
| male   | 16  |
| male   | 8   |
| male   | 7   |
| male   | 8   |
| male   | 4   |
| male   | 0   |
| male   | 2   |
| female | 4   |
| female | 7   |
| female | 13  |
| female | 8   |
| female | 8   |
| female | 4   |
| female | 14  |
| female | 5   |
| female | 6   |
| female | 4   |
| female | 12  |
| female | 9   |
| female | 9   |
| female | 9   |
| female | 8   |
| female | 12  |
| female | 4   |
| female | 8   |
| female | 8   |
| female | 4   |
| female | 11  |
| female | 6   |
| female | 15  |
| female | 9   |
| female | 8   |
| female | 14  |
| female | 9   |
| female | 8   |
| female | 9   |
| female | 7   |
| female | 12  |
| female | 11  |
| female | 7   |
| female | 4   |
| female | 10  |
| female | 7   |
| female | 8   |
| female | 8   |
| female | 7   |
| female | 9   |
| female | 10  |
| female | 16  |
| female | 14  |
| female | 15  |
| female | 10  |
| female | 4   |
| female | 6   |
| female | 3   |
| female | 9   |
| female | 3   |
| female | 10  |
| female | 3   |
| female | 8   |

```{r, echo = FALSE, message = FALSE, warning = FALSE}
dental <- data.frame(
"grp" = c(rep("m",34), rep("f",54)),
"score" = c(8,6,4,2,10,5,6,6,19,4,10,4,10,12,7,2,5,1,8,2,0,7,6,4,4,11,2,16,8,7,8,4,0,2,
4,7,13,4,8,8,4,14,5,6,4,12,9,9,9,8,12,4,8,8,4,11,6,15,9,8,14,9,8,9,7,12,11,7,4,10,7,8,8,7,9,10,16,14,15,10,4,6,3,9,3,10,3,8)
)
```

Use an asymptotic WMW test to determine whether the DMF score differs significantly between males and females.

```{r,echo = FALSE, message = FALSE, warning = FALSE}
dental_wrs <- wilcox.test(
  x = dental$score[dental$grp == "m"],
  y = dental$score[dental$grp == "f"], paired = FALSE,
  exact = FALSE, alternative = "two.sided")
```

------------------------------------------------------------------------

# 3. Comparing more than two related samples

## ONE

A graduate student performed a pilot study for his dissertation. He wanted to examine the effects of animal companionship on elderly males. He selected 10 male participants from a nursing home. Then he used an ABAB research design, where A represented a week with the absence of a cat and B represented a week with the presence of a cat. At the end of each week, he administered a 20-point survey to measure the quality of life satisfaction. The survey results are presented in `Table 5.9`

| **participant** | **week 1** | **week 2** | **week 3** | **week 4** |
|-------------|--------|--------|--------|--------|
| 1           | 7      | 6      | 8      | 9      |
| 2           | 9      | 8      | 10     | 1      |
| 3           | 15     | 18     | 16     | 17     |
| 4           | 7      | 6      | 8      | 9      |
| 5           | 7      | 8      | 10     | 11     |
| 6           | 10     | 14     | 13     | 11     |
| 7           | 12     | 19     | 11     | 13     |
| 8           | 7      | 4      | 2      | 5      |
| 9           | 8      | 7      | 9      | 5      |
| 10          | 12     | 16     | 14     | 15     |

: Table 5.9

```{r, echo = FALSE, message = FALSE, warning = FALSE}
buddy <- data.frame(
  "participant" = rep(1:10, each = 4),
  "week" = rep(1:4, times = 10),
  "score" = c(7,6,8,9, 9,8,10,1, 15,18,16,17, 7,6,8,9, 7,8,10,11, 
              10,14,13,11, 12,19,11,13, 7,4,2,5, 8,7,9,5, 12,16,14,15)
)
```

Use a **Friedman test** to determine if one or more of the groups are significantly different. Since this is a pilot study, use $\alpha = 0.10$. If a significant difference exists, use **Wilcoxon signed-rank tests** to identify which groups are significantly different. Use the **Bonferroni procedure** to limit the type I error rate. Report your findings

$${H}_{0}:\text{The quality of life satisfaction scores are identical across all four weeks} \\ {H}_{1}:\text{At least one week differs in quality of life satisfaction}$$

```{r, echo = FALSE, message = FALSE, warning = FALSE}
buddy_fr <- friedman.test(y = buddy$score, 
                          groups = buddy$week, 
                          blocks = buddy$participant)

cat(
  paste0("Friedman test","\n",
        "------------------------------------------------","\n",
        "p-value: ", round(buddy_fr$p.value,4), "\n",
        "Decision: ", ifelse(buddy_fr$p.value < 0.05,
                             yes = "Reject the null hypothesis",
                             no = "Fail to reject the null hypothesis"))
)
```
> A Friedman test was conducted to examine the effect of animal companionship (ABAB design) on quality of life satisfaction among $n=10$ elderly male participants across four weeks, with $\alpha = 0.05$. Median scores were $8.5, 8.0, 10.0, \text{ and } 10.0$ for weeks 1 through 4 respectively. The omnibus test was non-significant $({\chi^2}_{(3)}=`r buddy_fr$statistic`, p = `r round(buddy_fr$p.value,4)`)$ .We therefore fail to rejct ${H}_{0}.$ The data provides insufficient evidence of a significant difference in quality of life satisfaction across the four weeks. Since the Friedman test was non-significant, post-hoc Wilcoxon signed-rank comparisons were not warranted, even at $\alpha = 0.10$


## TWO

A physical education teacher conducted an action research project to examine a strength and conditioning program. Using 12 male participants, she measures the number of curl ups they could do in 1 minute. She measured their performance before the program. Then, she measured their performance at 1 month intervals. `Table 5.10` presents the performance results.

| participant | baseline | month 1 | month 2 |
|-------------|----------|---------|---------|
| 1           | 66       | 67      | 69      |
| 2           | 49       | 50      | 56      |
| 3           | 51       | 52      | 49      |
| 4           | 65       | 65      | 69      |
| 5           | 42       | 43      | 46      |
| 6           | 38       | 39      | 40      |
| 7           | 33       | 31      | 39      |
| 8           | 41       | 41      | 44      |
| 9           | 46       | 47      | 48      |
| 10          | 45       | 46      | 46      |
| 11          | 36       | 33      | 34      |
| 12          | 51       | 55      | 67      |

: Table 5.10: Number of curl ups in a minute

```{r, echo = FALSE, message = FALSE, warning = FALSE}
curlups <- data.frame(
  "participant" = rep(1:12, each = 3),
  "month" = rep(c("baseline", "month1", "month2"), times = 12),
  "score" = c(66,67,69,49,50,56,51,52,49,65,65,69,42,43,46,38,39,40,33,31,39,41,41,44,46,47,48,45,46,46,36,33,34,51,55,67)
)
```

$${H}_{0}:\text{The number of curl-ups is identical across all three time points} \\ \\ {H}_{1}:\text{Performance increases over time}$$

Use a **Friedman test** with $\alpha = 0.05$ to determine if one or more of the groups are significantly different. The teacher is expecting performance gains, so if a significant difference exists, use **one-tailed Wilcoxon signed-rank tests** to identify which groups are significantly different. Use the **Bonferroni** procedure to limit the type I error rate. Report your findings.

```{r, echo = FALSE, message = FALSE, warning = FALSE}
curlups_fr <- friedman.test(y = curlups$score, 
                            groups = curlups$month, 
                            blocks = curlups$participant)

cat(
  paste0("Friedman test","\n",
        "------------------------------------------------","\n",
        "p-value: ", round(curlups_fr$p.value,4), "\n",
        "Decision: ", ifelse(curlups_fr$p.value < 0.05,
                             yes = "Reject the null hypothesis",
                             no = "Fail to reject the null hypothesis"))
)
```
> A Friedman test was used to evaluate the effect of a strength and conditioning program on curl-up performance across three time points (baseline, month 1, month 2) for $n = 12$ male participants, with $\alpha = 0.05$. Median scores were $45.5, 46.5, \text{ and } 47.0$ respectively, suggesting a progressive increase. The Friedman test was _significant_ $({\chi^2}_{(2)}=`r round(curlups_fr$statistic,4)`, p = `r round(curlups_fr$p.value,4)`)$ indicating at least one time point differed. 

We therefore proceed to conduct post-hoc Wilcoxon signed-rank tests to establish the origin of the difference. Below are the post-hoc test results determined via the Bonferroni p-value $0.05/3=0.0167$; 


```{r, echo = FALSE, message = FALSE, warning = FALSE}
bonferroni_pvalue <- 0.05/3
# Bonferroni - Wilcoxon Sign rank test
## Baseline vs month 1 
curlup_wsr1 <- wilcox.test(
  x = curlups$score[curlups$month == "baseline"],
  y = curlups$score[curlups$month == "month1"], 
  alternative = "less", paired = TRUE)

## Baseline vs month 2
curlup_wsr2 <- wilcox.test(
  x = curlups$score[curlups$month == "baseline"],
  y = curlups$score[curlups$month == "month2"], 
  alternative = "less", paired = TRUE)

## month 1 vs month 2
curlup_wsr3 <- wilcox.test(
  x = curlups$score[curlups$month == "month1"],
  y = curlups$score[curlups$month == "month2"], 
  alternative = "less", paired = TRUE)


cat(
  paste0("Bonferroni-Wilcoxon Signed Rank test (baseline vs month 1)","\n",
         "------------------------------------------------------------","\n",
         "p-value: ", round(curlup_wsr1$p.value, 4), "\n",
         "Decision: ", ifelse(curlup_wsr1$p.value<bonferroni_pvalue,
                              yes = "Reject the null hypothesis",
                              no = "Fail to reject the null hypothesis"),"\n","\n",
         "Bonferroni-Wilcoxon Signed Rank test (baseline vs month 2)","\n",
         "-------------------------------------------------------------","\n",
         "p-value: ", round(curlup_wsr2$p.value, 4), "\n",
         "Decision: ", ifelse(curlup_wsr2$p.value<bonferroni_pvalue,
                              yes = "Reject the null hypothesis",
                              no = "Fail to reject the null hypothesis"),"\n","\n",
         "Bonferroni-Wilcoxon Signed Rank test (month 1 vs month 2)","\n",
         "-------------------------------------------------------------","\n",
         "p-value: ", round(curlup_wsr3$p.value, 4), "\n",
         "Decision: ", ifelse(curlup_wsr3$p.value<bonferroni_pvalue,
                              yes = "Reject the null hypothesis",
                              no = "Fail to reject the null hypothesis"))
)

```

> The comparison of _baseline_ vs _month 1_ was **non-significant** $(W = `r curlup_wsr1$statistic`, p = `r round(curlup_wsr1$p.value,4)`)$. However, _baseline_ vs _month 1_ was **significant** $(W = `r curlup_wsr2$statistic`, p = `r round(curlup_wsr2$p.value,4)`)$, as was _month 1_ vs _month 2_ $(W = `r curlup_wsr3$statistic`, p = `r round(curlup_wsr3$p.value,4)`)$. The conditioning program produced a significant improvement in curl-up performance by month 2, with meaningful gains observed between month 1 and month 2 as well, but no detectable change from baseline to month 1. 
 


## THREE

*(ANSM 6.8)*

At the beginning of a session, 12 names are read out in random order to 10 students. Four are names of prominent sporting personalities (Group A), four of national and international politicians (Group B) and four of local dignitaries (Group C). At the end of the session students are asked to recall as many of the names as possible. The numbers recalled were:

| student | group A | group B | group C |
|---------|---------|---------|---------|
| I       | 3       | 2       | 0       |
| II      | 1       | 1       | 0       |
| III     | 2       | 3       | 1       |
| IV      | 4       | 3       | 2       |
| V       | 3       | 2       | 2       |
| VI      | 1       | 0       | 0       |
| VII     | 3       | 2       | 4       |
| VIII    | 3       | 2       | 1       |
| IX      | 2       | 2       | 0       |
| X       | 4       | 3       | 2       |

```{r, echo = FALSE, message = FALSE, warning = FALSE}
remember <- data.frame(
  "student" = rep(as.roman(1:10), each = 3) |> as.character(),
  "grp" = rep(LETTERS[1:3], times = 10),
  "count" = c(3,2,0,1,1,0,2,3,1,4,3,2,3,2,2,
              1,0,0,3,2,4,3,2,1,2,2,0,4,3,2)
)
```

Rank the data within each block (student) and use a **Friedman test** to assess evidence of a difference between recall rates for the three groups. In particular, is the recall rate for group B and/or group C significantly lower than that for group A? Carry out an ANOVA on the given data. Do the conclusions agree with the Friedman test? If not, why not?

$${H}_{0}:\text{Recall rates are identical across all three name categories}\\ \\ {H}_{1}:\text{At least one category has a different recall rate}$$


```{r, echo = FALSE, message = FALSE, warning = FALSE}
remember_fr <- friedman.test(y = remember$count, 
                             groups = remember$grp,
                             blocks = remember$student)

cat(
  paste0("Friedman Test","\n",
        "-----------------","\n",
        "p-value: ", round(remember_fr$p.value,4), "\n",
        "Decision: ", ifelse(remember_fr$p.value < 0.05,
                             yes = "Reject the null hypothesis",
                             no = "Fail to reject the null hypothesis"))
)
```

> A Friedman test was performed to assess differences in recall rates across three name categories — sporting personalities (A), politicians (B), and local dignitaries (C) — among $ n=10 $ students, with $\alpha = 0.05$. Median recall counts were $3, 2, \text{ and } 1$ for groups A, B, and C respectively. We rejected the null hypothesis thus, test was _significant_ $({\chi^2}_{(2)}=`r remember_fr$statistic`, p = `r round(remember_fr$p.value,4)`)$.

One-way ANOVA

```{r, echo = FALSE, message = FALSE, warning = FALSE}
remember_aov <- aov(count ~ grp + student, data = remember) |> summary();remember_aov
```

> A one-way ANOVA with student as a blocking factor also yielded a significant group effect $(F=9.38, p = 0.002)$, consistent with the Friedman result. The two tests agree here because the data, while discrete and bounded, does not show severe departures from the ANOVA assumptions at this sample size.

Post-hoc Bonferroni Wilcoxon Signed Rank tests

```{r, echo = FALSE, message = FALSE, warning = FALSE}
# Bonferroni p-value
bonferroni_pvalue <- 0.05/length(unique(remember$grp))
# Wilcoxon Signed Rank tests
## group A vs group B
remember_wsr1 <- wilcox.test(
  x = remember$count[remember$grp == "A"],
  y = remember$count[remember$grp == "B"],
  paired = TRUE
)

## group A vs group C
remember_wsr2 <- wilcox.test(
  x = remember$count[remember$grp == "A"],
  y = remember$count[remember$grp == "C"],
  paired = TRUE
)

## group B vs group C
remember_wsr3 <- wilcox.test(
  x = remember$count[remember$grp == "B"],
  y = remember$count[remember$grp == "C"],
  paired = TRUE
)

cat(
  paste0("Bonferroni-Wilcoxon Signed Rank test (group A vs group B)","\n",
         "------------------------------------------------------------","\n",
         "p-value: ", round(remember_wsr1$p.value, 4), "\n",
         "Decision: ", ifelse(remember_wsr1$p.value<bonferroni_pvalue,
                              yes = "Reject the null hypothesis",
                              no = "Fail to reject the null hypothesis"),"\n","\n",
         "Bonferroni-Wilcoxon Signed Rank test (group A vs group C)","\n",
         "-------------------------------------------------------------","\n",
         "p-value: ", round(remember_wsr2$p.value, 4), "\n",
         "Decision: ", ifelse(remember_wsr2$p.value<bonferroni_pvalue,
                              yes = "Reject the null hypothesis",
                              no = "Fail to reject the null hypothesis"),"\n","\n",
         "Bonferroni-Wilcoxon Signed Rank test (group B vs group C)","\n",
         "-------------------------------------------------------------","\n",
         "p-value: ", round(remember_wsr3$p.value, 4), "\n",
         "Decision: ", ifelse(remember_wsr3$p.value<bonferroni_pvalue,
                              yes = "Reject the null hypothesis",
                              no = "Fail to reject the null hypothesis"))
)
```

> The comparison _A vs B_ was **non-significant** $(V = `r remember_wsr1$statistic`, p = `r round(remember_wsr1$p.value, 4)`)$ falling just above the adjusted threshold. The comparison _A vs C_ was **significant** $(V = `r remember_wsr2$statistic`, p = `r round(remember_wsr2$p.value, 4)`)$, confirming that group A was recalled significantly more than group C. The comparison _B vs C_ was **non-significant** $(V = `r remember_wsr3$statistic`, p = `r round(remember_wsr3$p.value, 4)`)$. _Sporting personalities_ were recalled at a significantly higher rate than _local dignitaries_, though no statistically significant difference was established between _politicians_ and _sporting personalities_ or _between politicians and local dignitaries_ after Bonferroni correction


## FOUR

*(ANSM 6.12)*

Four share tipsters are each asked to predict on 10 randomly selected days whether the London FTSE Index (commonly known as Footsie) will rise or fall on the following day. If they predict correctly this is scored as 1, if incorrectly as 0. Do the scores below indicate differences in tipsters’ ability to predict accurately?

```{r, echo = FALSE, message = FALSE, warning = FALSE}
ftse <- data.frame(
  "day" = rep(1:10, each = 4),
  "tipster" = rep(1:4, times = 10),
  "outcome" = c(1L,1L,1L,1L,0L,1L,1L,1L,0L,1L,0L,0L,1L,1L,1L,0L,1L,0L,1L,0L,1L,1L,1L,1L,1L,1L,1L,1L,0L,0L,1L,1L,1L,0L,0L,0L,1L,0L,1L,1L)
  )
```

| day | tipster 1 | tipster 2 | tipster 3 | tipster 4 |
|-----|-----------|-----------|-----------|-----------|
| 1   | 1         | 1         | 1         | 1         |
| 2   | 0         | 1         | 1         | 1         |
| 3   | 0         | 1         | 0         | 0         |
| 4   | 1         | 1         | 1         | 0         |
| 5   | 1         | 0         | 1         | 0         |
| 6   | 1         | 1         | 1         | 1         |
| 7   | 1         | 1         | 1         | 1         |
| 8   | 0         | 0         | 1         | 1         |
| 9   | 1         | 0         | 0         | 0         |
| 10  | 1         | 0         | 1         | 1         |

$${H}_{0}:\text{There is no difference in prediction accuracy among the four tipsters} \\ \\ {H}_{1}:\text{At least on tipster differs in in prediction theory}$$

```{r, echo = FALSE, message = FALSE, warning = FALSE}
ftse_cochran <- CochranQTest(y = ftse$outcome,
                             g = ftse$tipster,
                             blocks = ftse$day)


cat(
  paste0("Cochran's Q test","\n",
        "------------------","\n",
        "p-value: ", round(ftse_cochran$p.value,4), "\n",
        "Decision: ", ifelse(ftse_cochran$p.value < 0.05,
                             yes = "Reject the null hypothesis",
                             no = "Fail to reject the null hypothesis"))
)
```
> Cochran's Q test was applied to binary prediction outcomes (1 = correct, 0 = incorrect) from four share tipsters over 10 days, with $\alpha = 0.05$
. Correct prediction counts were $7, 6, 8, \text{ and } 6$ out of $10$ for tipsters 1 through 4 respectively. The test was **non-significant** $({Q}_{(3)} = `r ftse_cochran$statistic`,p = `r round(ftse_cochran$p.value,4)`$
.We fail to reject ${H}_{0}$ — there is no sufficient evidence of a difference in prediction accuracy among the four tipsters

## FIVE

*(ANSM 6.14)*

Berry (1987) gives the following data for numbers of premature ventricular contractions per hour for 12 patients with cardiac arrhythmias when each is treated with 3 drugs A, B, C

```{r, echo = FALSE, message = FALSE, warning = FALSE}
ventricle <- data.frame(
"patient" = rep(1:12, each = 3),
"drug" = rep(LETTERS[1:3], times = 12),
"count" = c(170,7,0,19,1.4,6,187,205,18,10,0.3,1,216,0.2,22,49,33,30,7,37,3,474,9,5,0.4,0.6,0,1.4,63,36,27,145,26,29,0,0))
```

| patient | drug A | drug B | drug C |
|---------|--------|--------|--------|
| 1       | 170    | 7      | 0      |
| 2       | 19     | 1.4    | 6      |
| 3       | 187    | 205    | 18     |
| 4       | 10     | 0.3    | 1      |
| 5       | 216    | 0.2    | 22     |
| 6       | 49     | 33     | 30     |
| 7       | 7      | 37     | 3      |
| 8       | 474    | 9      | 5      |
| 9       | 0.4    | 0.6    | 0      |
| 10      | 1.4    | 63     | 36     |
| 11      | 27     | 145    | 26     |
| 12      | 29     | 0      | 0      |

Use a Friedman test to investigate differences in response between drugs. In particular, is there evidence of a difference between drug A and drug B?

$${H}_{0}:\text{The distribution of premature ventricular contractions is identical across drugs A,B and C} \\ \\ {H}_{1}:\text{At least one drug produces a different contraction rate}$$

```{r, echo = FALSE, message = FALSE, warning = FALSE}
ventricle_fr <- friedman.test(count~drug|patient, data = ventricle)

cat(
  paste0("Friedman test","\n",
        "------------------","\n",
        "p-value: ", round(ventricle_fr$p.value,4), "\n",
        "Decision: ", ifelse(ventricle_fr$p.value < 0.05,
                             yes = "Reject the null hypothesis",
                             no = "Fail to reject the null hypothesis"))
)
```

> A Friedman test was conducted to investigate differences in premature ventricular contractions per hour among $12$ cardiac arrhythmia patients treated with three drugs (A, B, C), with $\alpha = 0.05$. Median contraction counts were $28, 8, \text{ and } 5.5$ for drugs A, B, and C respectively. The omnibus test was **significant** $({\chi^2}_{(2)} = `r round(ventricle_fr$p.value, 4)`, p = 0.018)$. We reject the null hypothesis, concluding that drug treatment has a significant effect on the rate of ventricular contraction per hour.

## SIX

*(ANSM 6.15)*

Cohen (1983) gives data for numbers of births in Israel for each day in 1975. We give below data for numbers of births on each day in the 10th, 20th, 30th and 40th weeks of the year

| week | day of the week | number of births |
|------|-----------------|------------------|
| 10   | Monday          | 108              |
| 10   | Tuesday         | 106              |
| 10   | Wednesday       | 100              |
| 10   | Thurdsay        | 85               |
| 10   | Friday          | 85               |
| 10   | Saturday        | 92               |
| 10   | Sunday          | 96               |
| 20   | Monday          | 82               |
| 20   | Tuesday         | 99               |
| 20   | Wednesday       | 89               |
| 20   | Thurdsay        | 125              |
| 20   | Friday          | 74               |
| 20   | Saturday        | 85               |
| 20   | Sunday          | 100              |
| 30   | Monday          | 96               |
| 30   | Tuesday         | 101              |
| 30   | Wednesday       | 108              |
| 30   | Thursday        | 103              |
| 30   | Friday          | 108              |
| 30   | Saturday        | 96               |
| 30   | Sunday          | 110              |
| 40   | Monday          | 124              |
| 40   | Tuesday         | 106              |
| 40   | Wednesday       | 111              |
| 40   | Thursday        | 115              |
| 40   | Friday          | 99               |
| 40   | Saturday        | 96               |
| 40   | Sunday          | 111              |

```{r, echo = FALSE, message = FALSE, warning = FALSE}
births <- data.frame(
"week" = rep(c(10,20,30,40), each = 7),
"day" = rep(c("Monday","Tuesday","Wednesday","Thursday","Friday","Saturday", "Sunday"), 4),
"freq" = c(108,106,100,85,85,92,96,
           82,99,89,125,74,85,100,
           96,101,108,103,108,96,110,
           124,106,111,115,99,96,111)
)
```

Perform Friedman analyses to determine whether the data indicate a difference in birth rate between days of the week that shows consistency over the four selected weeks

$${H}_{0}:\text{The distribution of daily bith counts is consistent across the four selected weeks} \\ \\ {H}_{1}:\text{At least one week shows a different pattern of daily births}$$

```{r, echo = FALSE, message = FALSE, warning = FALSE}
births_fr <- friedman.test(freq~week|day, data = births)

cat(
  paste0("Friedman test","\n",
        "------------------","\n",
        "p-value: ", round(births_fr$p.value,4), "\n",
        "Decision: ", ifelse(births_fr$p.value < 0.05,
                             yes = "Reject the null hypothesis",
                             no = "Fail to reject the null hypothesis"))
)

```
> A Friedman test was used to assess whether birth rates across days of the week were consistent over four selected weeks (weeks 10, 20, 30, 40) in Israel 1975, with $\alpha = 0.05$. The test was significant $({\chi^2}_{(3)}=`r births_fr$statistic`, p = `r round(births_fr$p.value,4)`)$. We reject ${H}_{0}$ .The data indicates that the pattern of daily births was inconsistent across the four selected weeks, suggesting week-to-week variation in the distribution of births by day.

## SEVEN

*(ANSM 6.16)*

Snee (1985) gives data on average liver weights per bird for chicks given three levels of growth promoter (none, low, high). Blocks correspond to different bird houses. Use a Friedman test to see if there is evidence of an effect of growth promoter

| house | none | low  | high |
|-------|------|------|------|
| 1     | 3.93 | 3.99 | 4.08 |
| 2     | 3.78 | 3.96 | 3.94 |
| 3     | 3.88 | 3.96 | 4.02 |
| 4     | 3.93 | 4.03 | 4.06 |
| 5     | 3.84 | 4.10 | 3.94 |
| 6     | 3.75 | 4.02 | 4.09 |
| 7     | 3.98 | 4.06 | 4.17 |
| 8     | 3.84 | 3.92 | 4.12 |

$${H}_{0}:\text{Average liver weight is identical across the three promoter dose levels}\\ \\ {H}_{1}:\text{At least one dose level produces a different liver weight}$$

```{r, echo = FALSE, message = FALSE, warning = FALSE}
liver <- data.frame(
"house" = rep(1:8, times = 3),
"level" = rep(c("none", "low", "high"), each = 8),
"weight" = c(3.93,3.78,3.88,3.93,3.84,3.75,3.98,3.84,
               3.99,3.96,3.96,4.03,4.10,4.02,4.06,3.92,
               4.08,3.94,4.02,4.06,3.94,4.09,4.17,4.12)
)

liver_fr <- friedman.test(weight~level|house, data = liver)

cat(
  paste0("Friedman test","\n",
        "------------------","\n",
        "p-value: ", round(liver_fr$p.value,4), "\n",
        "Decision: ", ifelse(liver_fr$p.value < 0.05,
                             yes = "Reject the null hypothesis",
                             no = "Fail to reject the null hypothesis"))
)
```


> A Friedman test was performed to assess the effect of growth promoter dose level (none, low, high) on average liver weight per chick across $n = 8$ bird houses, with $\alpha = 0.05$. Median liver weights were $3.860, 4.005, \text{ and } 4.070$ for none, low, and high doses respectively, suggesting a monotonic increase. The Friedman test was highly significant $({\chi^2}_{(2)}=`r liver_fr$statistic`, p = `r round(liver_fr$p.value,4)`)$ .We reject ${H}_{0}$

Since dose levels are ordered, a Page test is appropriate. Try this also

$${H}_{0}:\text{Liver weights are identically distriuted across dose levels}\\ \\ {H}_{1}:\text{Liver weights increase monotonically with dose level}$$

```{r, echo = FALSE,message = FALSE, warning = FALSE}
liver_jt <- JonckheereTerpstraTest(weight~level, data = liver, alternative = "two.sided")

cat(
  paste0("Jonckheere-Terpstra test","\n",
        "------------------------------","\n",
        "p-value: ", round(liver_jt$p.value,4), "\n",
        "Decision: ", ifelse(liver_jt$p.value < 0.05,
                             yes = "Reject the null hypothesis",
                             no = "Fail to reject the null hypothesis"))
)
```
> Since the dose levels are ordered, a Jonckheere-Terpstra directional analysis was also conducted. Pairwise results confirmed a significant increase from **none** to **low** $(W = 59, p = 0.003)$ and from **none** to **high** $(W = 62, p = 0.001). The comparison **low** to **high** was **non-significant** $(W = 43,p = 0.134)$, suggesting the most pronounced effect occurs between the **no-dose** and **low-dose** conditions, with diminishing gains at higher doses. Overall, the data provides strong evidence of a dose-ordered increase in average liver weight attributable to the growth promoter



------------------------------------------------------------------------

# 4. Comparing more than two unrelated samples

## ONE

A researcher conducted a study with *n = 15* participants to investigate strength gains from exercise. The participants were divided into three groups and given one of three treatments. Participants' strength gains were measured and ranked. The rankings are presented in `Table 6.8` below

| I   | II  | III |
|-----|-----|-----|
| 7   | 13  | 12  |
| 2   | 1   | 5   |
| 4   | 7   | 16  |
| 11  | 8   | 9   |
| 15  | 3   | 14  |

: Table 6.8

```{r, echo = FALSE, message = FALSE, warning = FALSE}
strength <- data.frame(
  "treatment" = rep(c("I","II","III"), each = 5),
  "str" = c(7,2,4,11,15, 13,1,7,8,3, 12,5,16,9,14)
)
```

Use a **Kruskal-Wallis H test** with $\alpha = 0.05$ to determine if one or more of the groups were significantly different. If a significant difference exists, use a **two-tailed Mann-Whitney U test** or a **two-sample Kolmogorov-Smirnov test** to identify which groups are significantly different. Use the **Bonferroni** procedure to limit the type I error rate. Report your findings

$$
{H}_{0}: \text{The perceived effectiveness rankings are identical across all three attractiveness groups}\\ \\
{H}_{1}:\text{At least one attractiveness group differs in perceived effectiveness rankings}
$$


```{r, echo = FALSE, message = FALSE, warning = FALSE}
str_kw <- kruskal.test(x = strength$str, g = strength$treatment)

cat(
  paste0("Kruskal-Wallis Test","\n",
        "------------------------------------------------","\n",
        "p-value: ", round(str_kw$p.value,4), "\n",
        "Decision: ", ifelse(str_kw$p.value < 0.05,
                             yes = "Reject the null hypothesis",
                             no = "Fail to reject the null hypothesis"))
)
```
> A Kruskal-Wallis H test was conducted to compare strength gains across three treatment groups (I, II, and III) among \(\mathit{n=15}\) participants (5 per group), with \(\mathit{\alpha = 0.05}\). Strength gains were ranked globally, yielding rank sums of \({R}_{I}=36.5, {R}_{II}=30.5 \text{ and } {R}_{III}=53\). The test was \(\mathit{non-significant } ({\chi^2}_{(2)}=2.72, p=0.257)\). We therefore fail to reject \({H}_{0}\) — there is insufficient evidence to conclude that any treatment group produced significantly different strength gains. Since the omnibus test was non-significant, post-hoc pairwise comparisons were not warranted; no further inference is made about individual group differences.

## TWO

A researcher investigated how physical attraction influences the perception among others of a person's effectiveness with difficult tasks. The photographs of 24 people were shown to a focus group. The group was asked to classify the photos into three groups: *very attractive, average* and *very unattractive*. Then, the group ranked the photographs according to their impressions of how capable they were of solving difficult problems. `Table 6.9` shows the classification and rankings of the people in the photos(1 = *most effective*, 24 = *least effective*)

| very attractive | average | very unattractive |
|-----------------|---------|-------------------|
| 1               | 3       | 11                |
| 2               | 4       | 15                |
| 5               | 8       | 16                |
| 6               | 9       | 18                |
| 7               | 13      | 20                |
| 10              | 14      | 21                |
| 12              | 19      | 23                |
| 17              | 22      | 24                |

: Table 6.9

```{r, echo = FALSE, message = FALSE, warning = FALSE}
attraction <- data.frame(
  "group" = rep(c("very_attractive", "average", "very_unattractive"), each = 8),
  "ranking" = c(1,2,5,6,7,10,12,17,
                3,4,8,9,13,14,19,22,
                11,15,16,18,20,21,23,24)
)
```

Use a **Kruskal-Wallis H test** with $\alpha = 0.05$ to determine if one or more of the groups are significantly different. If a significant difference exists, use **two-tailed Mann-Whitney U tests** to identify which groups are significantly different. Use the **Bonferroni** procedure to limit the type I error rate. Report you findings.


$${H}_{0}: \text{The perceived effectiveness rankings are identical across all three attractiveness groups} \\ \\ 
{H}_{1}: \text{At least one attractiveness group differs in perceived effectiveness rankings}$$


```{r, echo = FALSE, message = FALSE, warning = FALSE}
attraction_kw <- kruskal.test(x = attraction$ranking, g = attraction$group)


cat(
  paste0("Kruskal-Wallis Test","\n",
        "------------------------------------------------","\n",
        "p-value: ", round(attraction_kw$p.value,4), "\n",
        "Decision: ", ifelse(attraction_kw$p.value < 0.05,
                             yes = "Reject the null hypothesis",
                             no = "Fail to reject the null hypothesis"))
)
```

> A Kruskal-Wallis H test was performed to compare perceived effectiveness rankings (1=most effective, 24=least effective) A Kruskal-Wallis H test was performed to compare perceived effectiveness rankings across three attractiveness classifications — *very attractive*, *average*, and *very unattractive* - among \(n=24\) individuals, with \(\alpha=0.05.\) Mean rankings were \(7.5, 11.5 \text{ and }18.5\) respectively, suggesting a **systematic gradient**. The test was significant \( \mathit{({\chi^2}_{(2)}=9.92, p=0.007) } \) indicating that at least one group differed. We reject the null hypothesis. 

Now that we have established the existence of a significance in perceived ranking amongst the three attractiveness groups, we conduct the Mann-Whitney tests to determine its origin. The Bonferroni procedure adjusts our critical p-value to \(\mathit{0.05/3=0.0167}\). Below are the test results;   

```{r, echo = FALSE, message = FALSE, warning = FALSE}
# Adjusted Bonferroni p-value
bonferroni_pvalue <- 0.05/3

# Mann-Whitney comparison 1: very attractive vs average
attraction_mw1 <- wilcox.test(
  x = attraction$ranking[attraction$group == "very_attractive"],
  y = attraction$ranking[attraction$group == "average"],
  paired = FALSE, exact = FALSE)

# Mann-Whitney comparison 1: very attractive vs very unattractive
attraction_mw2 <- wilcox.test(
  x = attraction$ranking[attraction$group == "very_attractive"],
  y = attraction$ranking[attraction$group == "very_unattractive"],
  paired = FALSE, exact = FALSE)

# Mann-Whitney comparison 1: average vs very unattractive
attraction_mw3 <- wilcox.test(
  x = attraction$ranking[attraction$group == "average"],
  y = attraction$ranking[attraction$group == "very_unattractive"],
  paired = FALSE, exact = FALSE)


cat(
  paste0("Bonferroni-Mann-Whitney U test (very attractive vs average)","\n",
         "------------------------------------------------------------","\n",
         "p-value: ", round(attraction_mw1$p.value, 4), "\n",
         "Decision: ", ifelse(attraction_mw1$p.value < bonferroni_pvalue,
                              yes = "Reject the null hypothesis",
                              no = "Fail to reject the null hypothesis"),"\n","\n",
         "Bonferroni-Mann-Whitney U test (very attractive vs very unattractive)","\n",
         "---------------------------------------------------------------------","\n",
         "p-value: ", round(attraction_mw2$p.value, 4), "\n",
         "Decision: ", ifelse(attraction_mw2$p.value < bonferroni_pvalue,
                              yes = "Reject the null hypothesis",
                              no = "Fail to reject the null hypothesis"),"\n","\n",
         "Bonferroni-Mann-Whitney U test (average vs very unattractive)","\n",
         "-------------------------------------------------------------","\n",
         "p-value: ", round(attraction_mw3$p.value, 4), "\n",
         "Decision: ", ifelse(attraction_mw3$p.value < bonferroni_pvalue,
                              yes = "Reject the null hypothesis",
                              no = "Fail to reject the null hypothesis"))
)
```

> The comparison between _very attractive_ and _average_ groups was \(\mathit{non-significant } \mathit{(W = `r round(attraction_mw1$statistic,4)`, p = `r round(attraction_mw1$p.value,4)`)}\).
> The comparison between _very attractive_ and _very unattractive_ groups was \(\mathit{significant } \mathit{(W = `r round(attraction_mw2$statistic,4)`, p = `r round(attraction_mw2$p.value,4)`)}\).
> The comparison between _average_ and _very unattractive_ groups was \(\mathit{non-significant } \mathit{(W = `r round(attraction_mw3$statistic,4)`, p = `r round(attraction_mw3$p.value,4)`)}\). The data therefore provides sufficient evidence that individuals perceived as very attractive were ranked significantly more effective at solving difficult problems than those perceived as very unattractive



## THREE

*(ANSM 6.17)*

Lubischew (1962) gives measurements of maximum head width in units of 0.01 mm for three species of Chaetocnema. Part of his data is given below. Use a Kruskal–Wallis test to see if there is a species difference in head widths

```{r, echo = FALSE, message = FALSE, warning = FALSE}
chaetocnema <- data.frame(
"species" = c(rep(1, 10), rep(2,11), rep(3, 8)),
"hwidth" = c(53,50,52,50,49,47,54,51,52,57,
             49,49,47,54,43,51,49,51,50,46,49,
             58,51,45,53,49,51,50,51)
)
```

| species 1 | species 2 | species 3 |
|-----------|-----------|-----------|
| 53        | 49        | 58        |
| 50        | 49        | 51        |
| 52        | 47        | 45        |
| 50        | 54        | 53        |
| 49        | 43        | 49        |
| 47        | 51        | 51        |
| 54        | 49        | 50        |
| 51        | 51        | 51        |
| 52        | 50        |           |
| 57        | 46        |           |
|           | 49        |           |




$${H}_{0}:\text{The distribution of maximum head widths is identical across all three \textit{Chaetocnema} species} \\ \\ {H}_{1}:\text{At least one species differs in head width distribution}$$


```{r, echo = FALSE, message = FALSE, warning = FALSE}
chaetocnema_kr <- kruskal.test(hwidth~species, data = chaetocnema)

cat(
  paste0("Kruskal-Wallis Test","\n",
        "------------------------------------------------","\n",
        "p-value: ", round(chaetocnema_kr$p.value,4), "\n",
        "Decision: ", ifelse(chaetocnema_kr$p.value < 0.05,
                             yes = "Reject the null hypothesis",
                             no = "Fail to reject the null hypothesis"))
)
```


> A Kruskal-Wallis H test was used to assess species differences in maximum head width _(units:0.01mm)_ across three species of _Chaetocnema_ \({n}_{1}=10,{n}_{2}=11 \text{ and }{n}{3}=8\), with \(\alpha =0.05.\) Median head widths were \(51.5, 49.0, \text{ and } 51.0\) for species 1, 2, and 3 respectively.The test was \(\mathit{non-significant } ({\chi^2}_{2}=4.44 p = `r round(chaetocnema_kr$p.value, 4)`).\) We fail to reject the null hypothesis since the data does not provide sufficient evidence of a species difference in head widths at the 5% significance level.


------------------------------------------------------------------------

# 5. Tests for Nominal scale data

## ONE

A researcher wishes to determine if there is an association between the level of a teacher’s education and his/her job satisfaction. He surveyed 158 teachers. The frequencies of the corresponding results are displayed in `Table 8.19`

```{r, echo = FALSE, message = FALSE, warning = FALSE}
tsatisfaction <- matrix(
c(60,41,19,10,13,15), byrow = TRUE, nrow = 2, 
dimnames = list(c("satisfied", "unsatisfied"),
                c("Bachelor's degree", "Master's degree", "Post-Master's degree")) 
)

```

|             | Bachelor's degree | Master's degree | Post-Master's degree |
|-------------|-------------------|-----------------|----------------------|
| satisfied   | 60                | 41              | 19                   |
| unsatisfied | 10                | 13              | 15                   |

: Table 8.19

First, use a ${\chi}^{2}$-test for independence with $\alpha = 0.05$ to determine if there is an association between level of education and job satisfaction. Then, determine the effect size for the association. Report your findings

$${H}_{0}:\text{Level of education and job satisfaction are independent}
$$ $${H}_{1}:\text{Level of education and job satisfaction are associated}$$

```{r, echo = FALSE, message = FALSE, warning = FALSE}
# Chi-square approximation
tsatisfaction_chi <- chisq.test(tsatisfaction, 
                                correct = FALSE)

cat(
  paste0("Test of independence (Chi-Square approximation)","\n",
        "------------------------------------------------","\n",
        "p-value: ", round(tsatisfaction_chi$p.value,4), "\n",
        "Decision: ", ifelse(tsatisfaction_chi$p.value < 0.05,
                             yes = "Reject the null hypothesis",
                             no = "Fail to reject the null hypothesis"))
)
```

> In conclusion, education level and job satisfaction are statistically associative at the 5% significance level.

## TWO

A professor gave her class a 10-item survey to determine the students’ satisfaction with the course. Survey question responses were measured using a five-point Likert scale. The survey had a score range from +20 to −20. Table 8.20 shows the scores of the students in a class of 13 students who rated the professor

| gender | score | satisfaction |
|--------|-------|--------------|
| male   | +12   | \+           |
| male   | +6    | \+           |
| male   | -5    | \-           |
| male   | -10   | \-           |
| male   | +17   | \+           |
| male   | +4    | \+           |
| female | -2    | \-           |
| female | -13   | \-           |
| female | +10   | \+           |
| female | -8    | \-           |
| female | -11   | \-           |
| female | -4    | \-           |
| female | -14   | \-           |

```{r, echo = FALSE, message = FALSE, warning = FALSE}
course_satisfaction <- data.frame(
  "gender"=c(rep("male",6),rep("female", 7)),
  "satisfaction"=c("p","p","n","n","p","p","n","n","p","n","n","n","n")
)
```

Use a Fisher exact test with $\alpha =  0.05$ to determine if there is an association between gender and course satisfaction of the professor’s class. Then, determine the effect size for the association. Report your findings.

$${H}_{0}: \text{Gender and course satisfaction are independent}$$ $${H}_{0}: \text{Gender and course satisfaction are associated}$$

```{r, echo = FALSE, message = FALSE, warning = FALSE}
course_matrix <- matrix(
c(course_satisfaction[course_satisfaction$gender=="male" & course_satisfaction$satisfaction == "p",] |> nrow(),
 course_satisfaction[course_satisfaction$gender=="male" & course_satisfaction$satisfaction == "n",] |> nrow(),
 course_satisfaction[course_satisfaction$gender=="female" & course_satisfaction$satisfaction == "p",] |> nrow(),
 course_satisfaction[course_satisfaction$gender=="female" & course_satisfaction$satisfaction == "n",] |> nrow()
    ), 
  byrow = TRUE, ncol = 2, nrow = 2, 
  dimnames = list(c("male", "female"),
                  c("positive", "negative"))
)

# Fisher's Excat Test
course_safisfaction_chi <- fisher.test(course_matrix)

cat(
  paste0("Test of independence (Fisher's Exact Test)","\n",
        "------------------------------------------------","\n",
        "p-value: ", round(course_safisfaction_chi$p.value,4), "\n",
        "Decision: ", ifelse(course_safisfaction_chi$p.value < 0.05,
                             yes = "Reject the null hypothesis",
                             no = "Fail to reject the null hypothesis"))
)
```

> There exists insufficient evidence against the null hypothesis of independence between gender and course satisfaction at 5% significance level

## THREE

*(ANSM 10.11)*

In an English parliamentary electoral constituency a random sample of 400 voters are classified by age and political affiliation as follows

```{r, echo = FALSE, message = FALSE, warning = FALSE}
voters <- matrix(
  c(31,32,39,34,16,19,25,31,36,27,58,52),
  byrow = TRUE, ncol = 4, nrow = 3,
  dimnames = list(c("Conservative","Liberal Democrat","Labour"),
                  c("30 or under", "31-40", "41-55", "56 or over")))
```

|                      |                 |           |           |                |
|---------------|---------------|---------------|---------------|---------------|
|                      | **30 or under** | **31-40** | **41-55** | **56 or over** |
| **Conservative**     | 31              | 32        | 39        | 34             |
| **Liberal Democrat** | 16              | 19        | 25        | 31             |
| **Labour**           | 36              | 27        | 58        | 52             |

Is there evidence of an association between political affiliation and age? It is generally (though not universally) accepted that the Conservative, Liberal Democrat and Labour parties represent an ordering of right, middle and left in the political spectrum

```{r, echo = FALSE, message = FALSE, warning = FALSE}
# Chi-Square approximation (3x4 table)
voters_chi <- chisq.test(voters, correct = FALSE)

cat(
  paste0("Test of independence (Chi-Square approximation)","\n",
        "------------------------------------------------","\n",
        "p-value: ", round(voters_chi$p.value,4), "\n",
        "Decision: ", ifelse(voters_chi$p.value < 0.05,
                             yes = "Reject the null hypothesis",
                             no = "Fail to reject the null hypothesis"))
)

```

> Political affiliation and age are independent of each other at the 5% level

## FOUR

*(ANSM 10.12)*

Agresti (1984) quotes the following data on cross-classification of attitudes towards abortion and amounts of schooling based on the US General Social Survey, 1972. Test these data for evidence of association between attitudes and educational background

|                            | Disapprove | Neutral | Approve |
|----------------------------|------------|---------|---------|
| **Less than high school**  | 209        | 101     | 237     |
| **High school**            | 151        | 126     | 426     |
| **More than high school**  | 16         | 21      | 138     |

```{r, echo = FALSE, message = FALSE, warning = FALSE}
abortion <- matrix(
  c(209,101,237,151,126,426,16,21,138),
  byrow = TRUE, nrow = 3, ncol = 3,
  dimnames = list(
    c("Less than High school","High school","More than high school"),
    c("Disapprove", "Neutral", "Approve"))
)

# Chi-Square approximation (3x3 table)
abortion_chi <- chisq.test(abortion, correct = FALSE)

cat(
  paste0("Test of independence (Chi-Square approximation)","\n",
        "------------------------------------------------","\n",
        "p-value: ", round(abortion_chi$p.value,4), "\n",
        "Decision: ", ifelse(abortion_chi$p.value < 0.05,
                             yes = "Reject the null hypothesis",
                             no = "Fail to reject the null hypothesis"))
)
```
> At the 5% significance level, there exists sufficient evidence in favor of the claim of present association between subjects' attitudes towards abortion and educational background

# 6. Variable comparison

## ONE

*(ANSM 7.6)*

### a

A china manufacturer is investigating market response to seven designs of dinner set. The main markets are the *British* and *American*. To get some idea of preferences in the two markets a survey of 100 British and 100 American women is carried out and each woman is asked to rank the designs in order of preference from 1 for favorite to 7 for least acceptable. For each country the 100 rank scores for each design is totalled. The design with the lowest total is assigned rank 1, that with the next lowest total rank 2, and so on. Overall rankings for each country are

| Design | British rank | American rank |
|--------|--------------|---------------|
| A      | 1            | 3             |
| B      | 2            | 4             |
| C      | 3            | 1             |
| D      | 4            | 5             |
| E      | 5            | 2             |
| F      | 6            | 7             |
| G      | 7            | 6             |

Calculate the Spearman and Kendall correlation coefficients. Is there evidence of a positive association between orders of preference

$$\text{Spearman's rank correlation coefficeint}\\
{H}_{0}:{\rho}_{s}=0\\
{H}_{1}:{\rho}_{s}\neq0\\ \\ 
\text{Kendall's rank correlation coefficeint}\\
{H}_{0}:{\tau}_{s}=0\\
{H}_{1}:{\tau}_{s}\neq0$$
```{r, echo = FALSE, message = FALSE, warning = FALSE}
dinner_set <- data.frame(
  "country" = c(rep("UK",7),rep("US",7),rep("CAN",7),rep("AUS",7)),
  "design" = rep(LETTERS[1:7], 4),
  "rank" = c(1,2,3,4,5,6,7,3,4,1,5,2,7,6,5,3,2,4,1,6,7,3,1,4,2,7,6,5)
)

# A. USA vs UK
## Spearman rank correlation coefficient
dinner_set_UKUS_spearman <- cor.test(x = dinner_set$rank[dinner_set$country == "UK"],
                                     y = dinner_set$rank[dinner_set$country == "US"],
                                     method = "spearman", exact = TRUE,
                                     alternative = "greater")

## Kendall's correlation coefficient
dinner_set_UKUS_kendall <- cor.test(
  x = dinner_set$rank[dinner_set$country == "UK"],
  y = dinner_set$rank[dinner_set$country == "US"],
  method = "kendall", exact = TRUE,
  alternative = "greater")



cat(
  paste0("Spearman's correlation","\n",
        "--------------------------","\n",
        "value: ", round(dinner_set_UKUS_spearman$estimate,4), "\n",
        "p-value: ", round(dinner_set_UKUS_spearman$p.value,4), "\n",
        "Decision: ", ifelse(dinner_set_UKUS_spearman$p.value < 0.05,
                             yes = "Reject the null hypothesis",
                             no = "Fail to reject the null hypothesis"),
        "\n","\n",
        "====================================================","\n","\n",
        "Kendall's correlation","\n",
        "--------------------------","\n",
        "value: ", round(dinner_set_UKUS_kendall$estimate,4), "\n",
        "p-value: ", round(dinner_set_UKUS_kendall$p.value,4), "\n",
        "Decision: ", ifelse(dinner_set_UKUS_kendall$p.value < 0.05,
                             yes = "Reject the null hypothesis",
                             no = "Fail to reject the null hypothesis")
        )
)

```


> We fail to reject the null hypotheses for both rank coefficients. The data provides sufficient evidence in favor of the claim that the 7 dinner set designs have an insignificant difference in the order of preference from the _British_ and _American_ markets.   


### b

*(ANSM 7.7)*

The manufacturer above later decides to assess preferences in the *Canadian* and *Australian* markets by a similar method. Calculate the Spearman and Kendall correlation and coefficients. Is there evidence of a positive association between orders of preference.


```{r, echo = FALSE, message = FALSE, warning = FALSE}

# B. CAN vs AUS
## Spearman rank correlation coefficient
dinner_set_CANAUS_spearman <- cor.test(
  x = dinner_set$rank[dinner_set$country == "CAN"],
  y = dinner_set$rank[dinner_set$country == "AUS"],
  method = "spearman", exact = TRUE,
  alternative = "greater")

## Kendall's correlation coefficient
dinner_set_CANAUS_kendall <- cor.test(
  x = dinner_set$rank[dinner_set$country == "CAN"],
  y = dinner_set$rank[dinner_set$country == "AUS"],
  method = "kendall", exact = TRUE,
  alternative = "greater")



cat(
  paste0("Spearman's correlation","\n",
        "--------------------------","\n",
        "value: ", round(dinner_set_CANAUS_spearman$estimate,4), "\n",
        "p-value: ", round(dinner_set_CANAUS_spearman$p.value,4), "\n",
        "Decision: ", ifelse(dinner_set_CANAUS_spearman$p.value < 0.05,
                             yes = "Reject the null hypothesis",
                             no = "Fail to reject the null hypothesis"),
        "\n","\n",
        "====================================================","\n","\n",
        "Kendall's correlation","\n",
        "--------------------------","\n",
        "value: ", round(dinner_set_CANAUS_kendall$estimate,4), "\n",
        "p-value: ", round(dinner_set_CANAUS_kendall$p.value,4), "\n",
        "Decision: ", ifelse(dinner_set_CANAUS_kendall$p.value < 0.05,
                             yes = "Reject the null hypothesis",
                             no = "Fail to reject the null hypothesis")
        )
)
```

> We fail to reject the null hypotheses for both rank coefficients. The data provides sufficient evidence in favor of the claim that the 7 dinner set designs have an insignificant difference in the order of preference from the _Canadian_ and _Australian_ markets.

### c

*(ANSM 7.8)*

Perform an appropriate analysis of the ranked data for all four countries in Exercises 7.6 and 7.7 to assess the evidence for any overall concordance

#### UK-CAN

```{r, echo = FALSE, message = FALSE, warning = FALSE}

# C. UK vs CAN
## Spearman rank correlation coefficient
dinner_set_UKCAN_spearman <- cor.test(
  x = dinner_set$rank[dinner_set$country == "UK"],
  y = dinner_set$rank[dinner_set$country == "CAN"],
  method = "spearman", exact = TRUE,
  alternative = "greater")

## Kendall's correlation coefficient
dinner_set_UKCAN_kendall <- cor.test(
  x = dinner_set$rank[dinner_set$country == "UK"],
  y = dinner_set$rank[dinner_set$country == "CAN"],
  method = "kendall", exact = TRUE,
  alternative = "greater")



cat(
  paste0("Spearman's correlation","\n",
        "--------------------------","\n",
        "value: ", round(dinner_set_UKCAN_spearman$estimate,4), "\n",
        "p-value: ", round(dinner_set_UKCAN_spearman$p.value,4), "\n",
        "Decision: ", ifelse(dinner_set_UKCAN_spearman$p.value < 0.05,
                             yes = "Reject the null hypothesis",
                             no = "Fail to reject the null hypothesis"),
        "\n","\n",
        "====================================================","\n","\n",
        "Kendall's correlation","\n",
        "--------------------------","\n",
        "value: ", round(dinner_set_UKCAN_kendall$estimate,4), "\n",
        "p-value: ", round(dinner_set_UKCAN_kendall$p.value,4), "\n",
        "Decision: ", ifelse(dinner_set_UKCAN_kendall$p.value < 0.05,
                             yes = "Reject the null hypothesis",
                             no = "Fail to reject the null hypothesis")
        )
)
```
> We fail to reject the null hypotheses for both rank coefficients. The data provides sufficient evidence in favor of the claim that the 7 dinner set designs have an insignificant difference in the order of preference from the _British_ and Canadian markets.


#### UK-AUS

```{r, echo = FALSE, message = FALSE, warning = FALSE}

# D. UK vs AUS
## Spearman rank correlation coefficient
dinner_set_UKAUS_spearman <- cor.test(
  x = dinner_set$rank[dinner_set$country == "UK"],
  y = dinner_set$rank[dinner_set$country == "AUS"],
  method = "spearman", exact = TRUE,
  alternative = "greater")

## Kendall's correlation coefficient
dinner_set_UKAUS_kendall <- cor.test(
  x = dinner_set$rank[dinner_set$country == "UK"],
  y = dinner_set$rank[dinner_set$country == "AUS"],
  method = "kendall", exact = TRUE,
  alternative = "greater")



cat(
  paste0("Spearman's correlation","\n",
        "--------------------------","\n",
        "value: ", round(dinner_set_UKAUS_spearman$estimate,4), "\n",
        "p-value: ", round(dinner_set_UKAUS_spearman$p.value,4), "\n",
        "Decision: ", ifelse(dinner_set_UKAUS_spearman$p.value < 0.05,
                             yes = "Reject the null hypothesis",
                             no = "Fail to reject the null hypothesis"),
        "\n","\n",
        "====================================================","\n","\n",
        "Kendall's correlation","\n",
        "--------------------------","\n",
        "value: ", round(dinner_set_UKAUS_kendall$estimate,4), "\n",
        "p-value: ", round(dinner_set_UKAUS_kendall$p.value,4), "\n",
        "Decision: ", ifelse(dinner_set_UKAUS_kendall$p.value < 0.05,
                             yes = "Reject the null hypothesis",
                             no = "Fail to reject the null hypothesis")
        )
)
```
> We fail to reject the null hypotheses for both rank coefficients. The data provides sufficient evidence in favor of the claim that the 7 dinner set designs have an insignificant difference in the order of preference from the _British_ and _Australian_ markets.



#### USA-CAN

```{r, echo = FALSE, message = FALSE, warning = FALSE}

# E. USA vs CAN
## Spearman rank correlation coefficient
dinner_set_USACAN_spearman <- cor.test(
  x = dinner_set$rank[dinner_set$country == "US"],
  y = dinner_set$rank[dinner_set$country == "CAN"],
  method = "spearman", exact = TRUE,
  alternative = "greater")

## Kendall's correlation coefficient
dinner_set_USACAN_kendall <- cor.test(
  x = dinner_set$rank[dinner_set$country == "US"],
  y = dinner_set$rank[dinner_set$country == "CAN"],
  method = "kendall", exact = TRUE,
  alternative = "greater")



cat(
  paste0("Spearman's correlation","\n",
        "--------------------------","\n",
        "value: ", round(dinner_set_USACAN_spearman$estimate,4), "\n",
        "p-value: ", round(dinner_set_USACAN_spearman$p.value,4), "\n",
        "Decision: ", ifelse(dinner_set_USACAN_spearman$p.value < 0.05,
                             yes = "Reject the null hypothesis",
                             no = "Fail to reject the null hypothesis"),
        "\n","\n",
        "====================================================","\n","\n",
        "Kendall's correlation","\n",
        "--------------------------","\n",
        "value: ", round(dinner_set_USACAN_kendall$estimate,4), "\n",
        "p-value: ", round(dinner_set_USACAN_kendall$p.value,4), "\n",
        "Decision: ", ifelse(dinner_set_USACAN_kendall$p.value < 0.05,
                             yes = "Reject the null hypothesis",
                             no = "Fail to reject the null hypothesis")
        )
)
```

> We reject the null hypotheses for both rank coefficients. The data provides sufficient evidence against the initial claim that the 7 dinner set designs have an insignificant difference in the order of preference from the _American_ and _Canadian_ markets.


#### USA-AUS

```{r, echo = FALSE, message = FALSE, warning = FALSE}

# F. USA vs AUS
## Spearman rank correlation coefficient
dinner_set_USAAUS_spearman <- cor.test(
  x = dinner_set$rank[dinner_set$country == "US"],
  y = dinner_set$rank[dinner_set$country == "AUS"],
  method = "spearman", exact = TRUE,
  alternative = "greater")

## Kendall's correlation coefficient
dinner_set_USAAUS_kendall <- cor.test(
  x = dinner_set$rank[dinner_set$country == "US"],
  y = dinner_set$rank[dinner_set$country == "AUS"],
  method = "kendall", exact = TRUE,
  alternative = "greater")



cat(
  paste0("Spearman's correlation","\n",
        "--------------------------","\n",
        "value: ", round(dinner_set_USAAUS_spearman$estimate,4), "\n",
        "p-value: ", round(dinner_set_USAAUS_spearman$p.value,4), "\n",
        "Decision: ", ifelse(dinner_set_USAAUS_spearman$p.value < 0.05,
                             yes = "Reject the null hypothesis",
                             no = "Fail to reject the null hypothesis"),
        "\n","\n",
        "====================================================","\n","\n",
        "Kendall's correlation","\n",
        "--------------------------","\n",
        "value: ", round(dinner_set_USAAUS_kendall$estimate,4), "\n",
        "p-value: ", round(dinner_set_USAAUS_kendall$p.value,4), "\n",
        "Decision: ", ifelse(dinner_set_USAAUS_kendall$p.value < 0.05,
                             yes = "Reject the null hypothesis",
                             no = "Fail to reject the null hypothesis")
        )
)
```
> We fail to reject the null hypotheses for both rank coefficients. The data provides sufficient evidence in favor of the claim that the 7 dinner set designs have an insignificant difference in the order of preference from the _British_ and _Australian_ market.

## TWO

*(ANSM 7.9)*

In a pharmacological experiment involving \(\beta\) -blocking agents, Sweeting (1982) recorded for a control group of dogs, cardiac oxygen consumption (MVO) and left ventricular pressure (LVP). Calculate the Kendall and Spearman correlation coefficients. Is there evidence of correlation

| Dog | MVO | LVP |
|-----|-----|-----|
| A   | 78  | 32  |
| B   | 92  | 33  |
| C   | 116 | 45  |
| D   | 90  | 30  |
| E   | 106 | 38  |
| F   | 78  | 24  |
| G   | 99  | 44  |

\(\text{We test the relevant hypotheses:}\)

$$\text{Spearman's rank correlation coefficeint}\\
{H}_{0}:{\rho}_{s}=0\\
{H}_{1}:{\rho}_{s}\neq0\\ \\ 
\text{Kendall's rank correlation coefficeint}\\
{H}_{0}:{\tau}_{s}=0\\
{H}_{1}:{\tau}_{s}\neq0$$


```{r, echo = FALSE, message = FALSE, warning = FALSE}
oxygen <- data.frame(
  "dog" = rep(LETTERS[1:7], 2),
  "control" = c(rep("mvo",7), rep("lvp",7)),
  "measure" = c(78,92,116,90,106,78,99,
                32,33,45,30,38,24,44)
)

# Spearman's rank coefficient
oxygen_spearman <- cor.test(
  x = oxygen$measure[oxygen$control == "mvo"],
  y = oxygen$measure[oxygen$control == "lvp"],
  method = "spearman", exact = TRUE,
  )

# Kendall's coefficient
oxygen_kendall <- cor.test(
  x = oxygen$measure[oxygen$control == "mvo"],
  y = oxygen$measure[oxygen$control == "lvp"],
  method = "kendall", exact = TRUE,
  )

cat(
  paste0("Spearman's correlation","\n",
        "--------------------------","\n",
        "value: ", round(oxygen_spearman$estimate,4), "\n",
        "p-value: ", round(oxygen_spearman$p.value,4), "\n",
        "Decision: ", ifelse(oxygen_spearman$p.value < 0.05,
                             yes = "Reject the null hypothesis",
                             no = "Fail to reject the null hypothesis"),
        "\n","\n",
        "====================================================","\n","\n",
        "Kendall's correlation","\n",
        "--------------------------","\n",
        "value: ", round(oxygen_kendall$estimate,4), "\n",
        "p-value: ", round(oxygen_kendall$p.value,4), "\n",
        "Decision: ", ifelse(oxygen_kendall$p.value < 0.05,
                             yes = "Reject the null hypothesis",
                             no = "Fail to reject the null hypothesis")
        )
)
```
> For this pharmacological experiment involving 7 dogs (n=7) selected from the control group, we collect their data on **cardiac oxygen consumption** and **left ventricular pressure**. The paired observations from each of the 7 subjects is ranked for the purpose of the analysis. The **Spearman rank correlation coeficient** was significant \(\mathit{({r}_{s} = 0.9009, p < 0.05)}.\) **Kendall's coeficient** was also significant \(\mathit{({\tau} = 0.7807, p < 0.05)}\).This data shows that there exists a significant monotonic association between cardiac oxygen consumption and left ventricular pressure.      


## THREE

*(ANSM 7.10)*

Bardsley and Chambers (1984) gave numbers of beef cattle and sheep on 19 large farms in a region. Is there evidence of correlation

| cattle | sheep |
|--------|-------|
| 41     | 4716  |
| 0      | 4605  |
| 42     | 4951  |
| 15     | 2745  |
| 47     | 6592  |
| 0      | 8934  |
| 0      | 9165  |
| 0      | 5917  |
| 56     | 2618  |
| 67     | 1105  |
| 707    | 150   |
| 368    | 2005  |
| 231    | 3222  |
| 104    | 7150  |
| 132    | 8658  |
| 200    | 6304  |
| 172    | 1800  |
| 146    | 5270  |
| 0      | 1537  |

\(\text{We test the relevant hypotheses:}\)

$$\text{Spearman's rank correlation coefficeint}\\
{H}_{0}:{\rho}_{s}=0\\
{H}_{1}:{\rho}_{s}\neq0\\ \\ 
\text{Kendall's rank correlation coefficeint}\\
{H}_{0}:{\tau}_{s}=0\\
{H}_{1}:{\tau}_{s}\neq0$$

```{r, echo = FALSE, message = FALSE, warning = FALSE}
farm <- data.frame(
  "f" = 1:19,
  "cattle" = c(41,0,42,15,47,0,0,0,56,67,707,368,231,104,132,200,172,146,0),
  "sheep" = c(4716,4605,4951,2745,6592,8934,9165,5917,2618,1105,150,2005,3222,7150,8658,6304,1800,5270,1537)
)

# Spearman's rank correlation 
farm_spearman <- cor.test(
  x = farm$cattle, y = farm$sheep,
  method = "spearman", exact = FALSE
)

# Kendall's correlation 
farm_kendall <- cor.test(
  x = farm$cattle, y = farm$sheep,
  method = "kendall", exact = FALSE
)

cat(
  paste0("Spearman's correlation","\n",
        "--------------------------","\n",
        "value: ", round(farm_spearman$estimate,4), "\n",
        "p-value: ", round(farm_spearman$p.value,4), "\n",
        "Decision: ", ifelse(farm_spearman$p.value < 0.05,
                             yes = "Reject the null hypothesis",
                             no = "Fail to reject the null hypothesis"),
        "\n","\n",
        "====================================================","\n","\n",
        "Kendall's correlation","\n",
        "--------------------------","\n",
        "value: ", round(farm_kendall$estimate,4), "\n",
        "p-value: ", round(farm_kendall$p.value,4), "\n",
        "Decision: ", ifelse(farm_kendall$p.value < 0.05,
                             yes = "Reject the null hypothesis",
                             no = "Fail to reject the null hypothesis")
        )
)
```
> The number of beef cattle and sheep was collected from 19 randomly selected farms. These observations are ranked for subsequent analysis. The **Spearman rank correlation coefficeint** was insignificant \(\mathit{({r}_{s} = -0.331, p > 0.05)}\) .The same applies for the **Kendall's coefficient** \(\mathit{({\tau} = -0.235, p > 0.05)}\). There's an insignificant negative monotonic association between the number of cattle owned and sheep owned by the farms in that region.          




